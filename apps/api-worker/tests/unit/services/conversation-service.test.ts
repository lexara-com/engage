...
import { TestDataFactory, createTestAuthContext } from '../../setup';
...
describe('ConversationService', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    conversationService = new ConversationService(mockEnv as any);
  });
  ...
  describe('createConversation', () => {
    it('should create a new conversation and sync to index', async () => {
      ...
      expect(mockConversationDO.create).toHaveBeenCalledWith(
  ...
  describe('listConversations', () => {
    it('should query D1 index with proper filters', async () => {
      ...
      expect(mockD1Database.prepare).toHaveBeenCalledWith(
        expect.stringContaining('WHERE firmId = ? AND isDeleted = FALSE')
      );
    });
  ...
  describe('addMessage', () => {
    it('should add message to DO and trigger index sync', async () => {
      ...
      expect(result).toBeDefined();
      expect(result.messages).toHaveLength(2);
      expect(mockConversationDO.addMessage).toHaveBeenCalledWith(
        expect.objectContaining({
          content: 'Test message',
          role: 'user',
  ...
  describe('searchConversations', () => {
    ...
    it('should filter results by threshold', async () => {
      ...
      expect(mockD1Database.prepare).toHaveBeenCalledWith(
        expect.stringContaining('sessionId IN (?)')
      );
    });
  ...
  describe('getConversationAnalytics', () => {
    it('should aggregate conversation metrics from D1', async () => {
      ...
      mockD1Database.prepare().bind().all
        .mockResolvedValueOnce({                    // Practice areas
          results: [
  ...
  describe('Data Quality Score Calculation', () => {
    it('should calculate score based on goal completion and data completeness', () => {
      ...
      const service = conversationService as any;
  ...
  describe('Error Handling', () => {
    it('should handle DO operation failures gracefully', async () => {
      ...
      await expect(
        conversationService.getConversation('session_123')
    ...
    it('should handle D1 query failures gracefully', async () => {
      ...
      await expect(
        conversationService.listConversations('firm_test_001')
