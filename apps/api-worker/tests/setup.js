...
export async function seedTestData(db) {
  ...
  `).bind(testFirmId, testSessionId, testUserId, 'John Doe', 'john@example.com', 'personal_injury', 'active', 'pre_login', 'pending', 3, 5, 75, new Date().toISOString(), new Date().toISOString(), false).run();
    // Seed users
    await db.prepare(`
  ...
  `).bind(testFirmId, testUserId, 'auth0|test123', 'test@lawfirm.com', 'Test Attorney', 'attorney', 'active', new Date().toISOString(), 1, new Date().toISOString()).run();
    // Seed firm settings
    await db.prepare(`
    INSERT INTO firm_config_index (
      firmId, configKey, configValue, configType, category, lastModified, modifiedBy
    ) VALUES (?, ?, ?, ?, ?, ?, ?)
  `).bind(testFirmId, 'firm_name', 'Test Law Firm', 'string', 'basic_info', new Date().toISOString(), testUserId).run();
}
/**
 ...
 */
export function createTestAuthContext(overrides) {
    return {
        firm: {
