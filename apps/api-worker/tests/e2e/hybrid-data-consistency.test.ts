...
describe('Hybrid Data Consistency E2E Tests', () => {
  ...
  describe('Conversation Lifecycle Data Consistency', () => {
    it('should maintain consistency through complete conversation lifecycle', async () => {
      ...
      const doDetailResponse = await app.fetch(
        createTestRequest({
          method: 'GET',
      ...
      await waitFor(async () => {
        ...
        return conversations.some((c: any) => c.sessionId === sessionId);
      }, 10000);
      ...
      expect(updatedConversation.messages).toContainEqual(
      ...
      await waitFor(async () => {
        ...
        const { conversations } = await listResponse.json();
        const indexedConversation = conversations.find((c: any) => c.sessionId === sessionId);
      ...
      const assignmentResponse = await app.fetch(
        createTestRequest({
          method: 'PUT',
      ...
      const assignedDoResponse = await app.fetch(
        createTestRequest({
          method: 'GET',
          path: `/api/v1/firm/conversations/${sessionId}`,
          authToken
      ...
      await waitFor(async () => {
        ...
        return conversations.some((c: any) => 
          c.sessionId === sessionId && c.assignedTo === testUserId
        );
      }, 10000);
      ...
      await waitFor(async () => {
        ...
        if (doResponse.status !== 200 || listResponse.status !== 200) return false;
        ...
        return doFinalConversation.status === 'completed' &&
               completedConversations.some((c: any) => c.sessionId === sessionId);
      }, 10000);
    });
    ...
    it('should handle concurrent updates without data corruption', async () => {
      ...
      const concurrentMessages = [
        { content: 'Message 1', role: 'user' as const },
        { content: 'Message 2', role: 'assistant' as const },
        { content: 'Message 3', role: 'user' as const },
        { content: 'Message 4', role: 'assistant' as const },
        { content: 'Message 5', role: 'user' as const }
      ];

      const messagePromises = concurrentMessages.map((message, index) =>
        app.fetch(
          createTestRequest({
            method: 'POST',
            path: `/api/v1/firm/conversations/${sessionId}/messages`,
  ...
  describe('Analytics Data Consistency', () => {
    it('should maintain accurate analytics across data sources', async () => {
      ...
      const analyticsResponse = await app.fetch(
        createTestRequest({
          method: 'GET',
      ...
      const practiceAreaResponse = await app.fetch(
        createTestRequest({
          method: 'GET',
      ...
      const personalInjuryMetrics = practiceAreaMetrics.find(
        (m: any) => m.practiceArea === 'personal_injury'
      );
      const familyLawMetrics = practiceAreaMetrics.find(
        (m: any) => m.practiceArea === 'family_law'
      );
  ...
  describe('Search Index Consistency', () => {
    it('should maintain search index consistency with conversation updates', async () => {
      ...
      const searchableMessages = [
        { content: 'I was in a car accident on Highway 101', role: 'user' as const },
        { content: 'The other driver ran a red light', role: 'user' as const },
        { content: 'I suffered whiplash and back injuries', role: 'user' as const }
      ];
      ...
      await waitFor(async () => {
        ...
        if (searchResponse.status !== 200) return false;
  ...
  describe('Assignment Workflow Consistency', () => {
    it('should maintain assignment consistency across all data layers', async () => {
      ...
      await waitFor(async () => {
        const assignmentListResponse = await app.fetch(
          createTestRequest({
            method: 'GET',
        ...
        if (assignmentListResponse.status !== 200) return false;

        const { assignments } = await assignmentListResponse.json();
        return assignments.some((a: any) => 
          a.sessionId === sessionId && a.assignedTo === assigneeId
        );
      }, 10000);

      // Verify conversation shows as assigned
      await waitFor(async () => {
        const conversationResponse = await app.fetch(
          createTestRequest({
            method: 'GET',
            path: `/api/v1/firm/conversations/${sessionId}`,
            authToken
        ...
        if (conversationResponse.status !== 200) return false;
      ...
      const workloadResponse = await app.fetch(
        createTestRequest({
          method: 'GET',
      ...
      const userWorkload = workloadAnalysis.find((w: any) => w.assignedTo === assigneeId);
      ...
      const operations = [
        () => app.fetch(
          createTestRequest({
            method: 'POST',
            path: `/api/v1/firm/conversations/${sessionId}/messages`,
            body: { content: 'Resilience test message 1', role: 'user' as const },
            authToken
          }),
          testEnv.env
        ),
        ...
        () => app.fetch(
          createTestRequest({
            method: 'POST',
            path: `/api/v1/firm/conversations/${sessionId}/messages`,
            body: { content: 'Resilience test message 2', role: 'assistant' as const },
            authToken
          }),
          testEnv.env
        )
