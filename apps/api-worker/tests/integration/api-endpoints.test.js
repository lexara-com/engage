...
import { testEnv, createTestRequest, createTestJWT, TestDataFactory } from '../setup';
...
describe('API Endpoints Integration Tests', () => {
        ...
        describe('GET /api/v1/firm/conversations', () => {
            ...
            it('should filter conversations by status', async () => {
                ...
                if (data.conversations.length > 0) {
                    expect(data.conversations.every((c) => c.status === 'active')).toBe(true);
                }
            });
    ...
    describe('Rate Limiting', () => {
        it('should enforce rate limits for authenticated requests', async () => {
            ...
            const requests = Array(10).fill(null).map(() => createTestRequest({
                method: 'GET',
                path: '/api/v1/firm/conversations',
                authToken
            }));
            const responses = await Promise.all(requests.map(req => app.fetch(req, testEnv.env)));
